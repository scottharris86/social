class UserFriendship < ActiveRecord::Base
  ...
  def not_blocked
    if UserFriendship.exists?(user_id: user_id, friend_id: friend_id, state: 'blocked') || 
      UserFriendship.exists?(friend_id: user_id, user_id: friend_id, state: 'blocked')
  ...
  def accept_mutual_friendship!
    # Grab the mutual friendship and manually update the state
    # without the state machine so as to not invoke callbacks
